...
"$script_dir/packages.sh" --profile "$profile" "$mode"
profile_args=(--profile "$profile")
$dry_run && profile_args+=(--dry-run)
$template_mode && profile_args+=(--template-mode)
"$script_dir/install-profile.sh" "${profile_args[@]}"
if $dry_run; then
  echo "would verify commands and run profile-appropriate tests after hardware attachment"
else
  for command in doctor probe encode validate-output collect-evidence; do [[ -x "$root/bin/$command" ]] || { echo "missing command: $command" >&2; exit 69; }; done
